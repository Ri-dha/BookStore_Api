...
public class UserController: BaseController
{
    ...
    {
        var (user, error) = await _userServices.ChangePassword(changePasswordForm.OldPassword, changePasswordForm.NewPassword, id);
        if (error != null) return BadRequest(new { error });
        return Ok(user);
    }
    
    [HttpPut("update-admin/{id}")]
    public async Task<IActionResult> UpdateAdmin(Guid id, [FromBody] AdminUpdateForm adminUpdateForm)
    {
